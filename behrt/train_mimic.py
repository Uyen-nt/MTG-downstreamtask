...
# =======================
data = np.load("data/result/real_from_mtg.npz")
x = data["x"]
lens = data["lens"]
...
np.savez("data/result/mimic_train.npz", x=x[train_idx], lens=lens[train_idx])
np.savez("data/result/mimic_val.npz",   x=x[val_idx],   lens=lens[val_idx])
np.savez("data/result/mimic_test.npz",  x=x[test_idx],  lens=lens[test_idx])
...
# =======================
config = {
    'vocab_size': 2871,         # same ICD dimension for both real and synthetic
    'hidden_size': 288,
    'seg_vocab_size': 34,
    'age_vocab_size': 2,
...
for epoch in range(30):
    ...
    if avg_val_loss < best_val_loss:
        ...
        wait = 0
        torch.save(model.state_dict(), "behrt/result/best_val_model_real.pt")
        print(f"🔥 Best REAL model saved at epoch {epoch} — VAL loss improved to {best_val_loss:.4f}")
    else:
        wait += 1
...
# 6) FINAL TEST
# =======================
model.load_state_dict(torch.load("behrt/result/best_val_model_real.pt"))
model.eval()
...
print(f"\n🟨 FINAL TEST LOSS (REAL) = {avg_test_loss:.4f}")
