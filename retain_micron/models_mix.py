...
class Retain(nn.Module):
    def __init__(self, voc_size, emb_size=64, device=torch.device('cpu:0')):
        ...
        # input và output để dự đoán procedures
        # self.input_len = voc_size[0] + voc_size[1] + voc_size[2]
        # self.output_len = voc_size[2]

        # input và output dự đoán diagnoses
        self.input_len = voc_size[0]  # Chỉ dùng diag
        self.output_len = voc_size[0] # Dự đoán diagnosis lần sau
    ...
    def forward(self, input):
        # input: list of visits, mỗi visit là list diagnosis codes
        device = self.device
        max_len = max([len(visit) for visit in input])
        
        input_np = []
        for visit in input:
            codes = visit[:max_len] + [self.input_len] * (max_len - len(visit))
            input_np.append(codes)
            
        visit_emb = self.embedding(torch.LongTensor(input_np).to(device))  # (n_visit, max_len, emb)
        visit_emb = torch.sum(visit_emb, dim=1)  # (n_visit, emb)
        
        g, _ = self.alpha_gru(visit_emb.unsqueeze(0))
        h, _ = self.beta_gru(visit_emb.unsqueeze(0))
        
        attn_alpha = F.softmax(self.alpha_li(g.squeeze(0)), dim=0)
        attn_beta = torch.tanh(self.beta_li(h.squeeze(0)))
        
        context = torch.sum(attn_alpha * attn_beta * visit_emb, dim=0, keepdim=True)
        output = self.output(context)  # (1, n_diagnosis)
        return output
